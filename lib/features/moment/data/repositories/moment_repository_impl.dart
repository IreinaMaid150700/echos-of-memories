...
class MomentRepositoryImpl implements MomentRepository {
  ...
  Future<Either<Failure, List<MomentEntity>>> getMoments() async {
    try {
      ...
      return const Left(
        CacheFailure(message: 'Không tải được danh sách khoảnh khắc'),
      );
    }
  }
  ...
  @override
  Future<Either<Failure, MomentEntity>> createMoment(
  ...
  Future<List<TagEntity>> _tagsForMoment(String momentId) async {
    final rows =
        await (_db.select(
          _db.momentTagLinks,
        )..where((l) => l.momentId.equals(momentId))).join([
          innerJoin(
            _db.momentTags,
    ...
    return rows
        .map((row) => _tagToEntity(row.readTable(_db.momentTags)))
        .toList();
  }
  ...
  Future<Map<String, List<TagEntity>>> _tagsForMoments(
    ...
    final rows =
        await (_db.select(
          _db.momentTagLinks,
        )..where((l) => l.momentId.isIn(momentIds))).join([
          innerJoin(
            _db.momentTags,
            _db.momentTags.id.equalsExp(_db.momentTagLinks.tagId),
          ),
  ...
  MomentEntity _toEntity(Moment moment, List<TagEntity> tags) => MomentEntity(
    ...
    coverAssetId: moment.coverAssetId,
    tags: tags,
  );
